...
help:
	...
	@echo "make test-beads-setup  # BEADS_NATIVE_SETUP_TESTS=1 enables isolated bd characterization"
...
test-contract-check:
	@python3 -m unittest discover -s skills/contract-check/tests -p 'test_*.py'
...
test-second-opinion:
	...
	@python3 -m unittest discover -s skills/second-opinion/tests -p 'test_subscription_route_check.py'
...
.PHONY: test-beads-setup test-beads-migrate test-beads-migrate-cli

test-beads-setup:
	@python3 -m unittest discover -s skills/beads-setup/tests -p 'test_*.py'
