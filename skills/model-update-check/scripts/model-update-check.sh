...
jq -n \
  ...
  (($pi_version != "" and $latest_pi_version != "") and
    (($latest_pi_version | version_parts) > ($pi_version | version_parts))) as $pi_update_available |

  {
    ...
    findings: (
      [
        ...
        if $pi_update_available then
          {severity: "review", kind: "pi-update", current: $pi_version, candidate: $latest_pi_version, message: "a newer Pi release may contain fresher built-in model metadata"}
        else empty end
      ] +
      [
        $audited[] |
