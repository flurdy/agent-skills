...
- `--run {run_id}` → resolve the run and its exact commit directly:
  ...
  ```
  Record `headSha` as `{sha}` and retain the returned run and workflow identity. Skip the commit-based
  run listing below.
- PR number → use its merge commit (`gh pr view {n} --json mergeCommit --jq .mergeCommit.oid`).
- Explicit sha → use it unchanged.
- No target → resolve the latest default-branch commit:

```bash
~/.agents/skills/watch-actions-rollout/scripts/default-head-sha.sh        # fetches origin/main, prints its sha
```
...
```bash
~/.agents/skills/watch-actions-rollout/scripts/run-jobs.sh {run_id}
```
