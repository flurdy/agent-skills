...
|---|---|---|
| `agents-blocks` | A native exact edit removes one uniquely paired general block and/or one exact checked Codex block from regular `AGENTS.md`, preserving every byte outside selected spans | Linked/hardlinked/ambiguous path, malformed/duplicate markers, missing replacement guidance, recovery evidence or exact old/new preview |
