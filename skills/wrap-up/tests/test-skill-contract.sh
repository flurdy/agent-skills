...
assert_contains "$SKILL" 'Bash(~/.agents/skills/wrap-up/scripts/save-handoff.py:*)'
assert_contains "$SKILL" 'save_handoff'
assert_contains "$SKILL" 'Do not fall back to Bash, native file tools, a different directory, or `/grant-file` in Pi.'
assert_contains "$SKILL" '#### Other clients: use the helper under their own permission policy'
assert_not_contains "$SKILL" 'Write'
assert_not_contains "$SKILL" 'handoff-path.sh'
...
assert_contains "$SKILL" 'wrap-up-save/v1'
assert_contains "$SKILL" '"status": "saved"'
assert_contains "$SKILL" '--overwrite-sha256'
assert_contains "$SKILL" 'Never print `Saved to`'
assert_contains "$SKILL" 're-render the complete resume block'
