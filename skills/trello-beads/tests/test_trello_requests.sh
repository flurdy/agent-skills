...
grep -Fq 'New card' "$stdout" || fail 'card creation behavior was not preserved'
grep -Fq "Added label 'bead' to card" "$stdout" || fail 'label mutation behavior was not preserved'
grep -Fq 'New comment' "$stdout" || fail 'comment mutation behavior was not preserved'
